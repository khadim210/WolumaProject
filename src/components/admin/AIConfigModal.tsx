import React, { useState, memo } from 'react';
import { aiEvaluationService, AIProvider } from '../../services/aiEvaluationService';
import { Card, CardHeader, CardTitle, CardContent, CardFooter } from '../ui/Card';
import Button from '../ui/Button';
import { X, Save, Bot, Key, TestTube, Brain } from 'lucide-react';

interface AIConfigModalProps {
  isOpen: boolean;
  onClose: () => void;
}

const PROVIDER_MODELS: Record<string, { value: string; label: string }[]> = {
  gemini: [
    { value: 'gemini-2.5-flash-preview-04-17', label: 'Gemini 2.5 Flash Preview (Recommandé)' },
    { value: 'gemini-2.5-pro-preview-05-06', label: 'Gemini 2.5 Pro Preview (Plus puissant)' },
    { value: 'gemini-2.0-flash', label: 'Gemini 2.0 Flash' },
    { value: 'gemini-2.0-flash-lite', label: 'Gemini 2.0 Flash Lite (Économique)' },
    { value: 'gemini-1.5-pro', label: 'Gemini 1.5 Pro' },
  ],
  chatgpt: [
    { value: 'gpt-4.1', label: 'GPT-4.1 (Dernier - Haute performance)' },
    { value: 'gpt-4.1-mini', label: 'GPT-4.1 Mini (Recommandé - Économique)' },
    { value: 'gpt-4o', label: 'GPT-4o' },
    { value: 'gpt-4o-mini', label: 'GPT-4o Mini' },
    { value: 'o4-mini', label: 'o4-mini (Raisonnement)' },
    { value: 'o3-mini', label: 'o3-mini (Raisonnement compact)' },
  ],
  anthropic: [
    { value: 'claude-sonnet-4-5', label: 'Claude Sonnet 4.5 (Recommandé - Équilibré)' },
    { value: 'claude-opus-4-5', label: 'Claude Opus 4.5 (Plus puissant)' },
    { value: 'claude-haiku-3-5', label: 'Claude Haiku 3.5 (Rapide et économique)' },
    { value: 'claude-3-7-sonnet-20250219', label: 'Claude 3.7 Sonnet' },
  ],
  mistral: [
    { value: 'mistral-large-latest', label: 'Mistral Large (Recommandé)' },
    { value: 'mistral-medium-latest', label: 'Mistral Medium' },
    { value: 'mistral-small-latest', label: 'Mistral Small' },
    { value: 'open-mistral-nemo', label: 'Mistral Nemo (Open Source)' },
  ],
};

const DEFAULT_MODELS: Record<string, string> = {
  gemini: 'gemini-2.5-flash-preview-04-17',
  chatgpt: 'gpt-4.1-mini',
  anthropic: 'claude-sonnet-4-5',
  mistral: 'mistral-large-latest',
};

const AIConfigModal: React.FC<AIConfigModalProps> = memo(({ isOpen, onClose }) => {
  const [selectedProvider, setSelectedProvider] = useState<AIProvider>('mock');
  const [apiKey, setApiKey] = useState('');
  const [selectedModel, setSelectedModel] = useState('');
  const [isTestingConnection, setIsTestingConnection] = useState(false);
  const [testResult, setTestResult] = useState<{ success: boolean; message: string } | null>(null);

  const providers = [
    {
      id: 'mock' as AIProvider,
      name: 'Simulation (Mock)',
      description: 'Évaluation simulée pour les tests — aucune clé requise',
      icon: <TestTube className="h-5 w-5" />,
      requiresApiKey: false,
      color: 'text-gray-500',
    },
    {
      id: 'chatgpt' as AIProvider,
      name: 'OpenAI (GPT)',
      description: 'GPT-4.1, GPT-4o, o3/o4-mini',
      icon: <Bot className="h-5 w-5" />,
      requiresApiKey: true,
      color: 'text-green-600',
    },
    {
      id: 'anthropic' as AIProvider,
      name: 'Anthropic (Claude)',
      description: 'Claude Sonnet 4.5, Opus 4.5, Haiku 3.5',
      icon: <Brain className="h-5 w-5" />,
      requiresApiKey: true,
      color: 'text-orange-600',
    },
    {
      id: 'gemini' as AIProvider,
      name: 'Google (Gemini)',
      description: 'Gemini 2.5, 2.0 Flash',
      icon: <Bot className="h-5 w-5" />,
      requiresApiKey: true,
      color: 'text-blue-600',
    },
    {
      id: 'mistral' as AIProvider,
      name: 'Mistral AI',
      description: 'Mistral Large, Medium, Small',
      icon: <Bot className="h-5 w-5" />,
      requiresApiKey: true,
      color: 'text-red-600',
    },
  ];

  const handleProviderChange = (provider: AIProvider) => {
    setSelectedProvider(provider);
    setSelectedModel(DEFAULT_MODELS[provider] || '');
    setTestResult(null);
  };

  const handleSave = () => {
    aiEvaluationService.setProvider(selectedProvider, apiKey);
    if (selectedModel) {
      aiEvaluationService.setModel(selectedModel);
      localStorage.setItem('ai_model', selectedModel);
    }
    localStorage.setItem('ai_provider', selectedProvider);
    if (apiKey) {
      localStorage.setItem('ai_api_key', apiKey);
    }
    onClose();
  };

  const handleTestConnection = async () => {
    setIsTestingConnection(true);
    setTestResult(null);

    try {
      const testRequest = {
        projectData: {
          title: 'Projet de test',
          description: 'Description de test pour vérifier la connexion API',
          budget: 100000,
          timeline: '12 mois',
          tags: ['test'],
        },
        evaluationCriteria: [
          {
            id: 'test1',
            name: 'Test Critère',
            description: 'Critère de test',
            maxScore: 20,
            weight: 100,
          },
        ],
      };

      const currentProvider = localStorage.getItem('ai_provider');
      const currentApiKey = localStorage.getItem('ai_api_key');
      const currentModel = localStorage.getItem('ai_model');

      aiEvaluationService.setProvider(selectedProvider, apiKey);
      if (selectedModel) aiEvaluationService.setModel(selectedModel);

      await aiEvaluationService.evaluateProject(testRequest);

      if (currentProvider) {
        aiEvaluationService.setProvider(currentProvider as AIProvider, currentApiKey || '');
        if (currentModel) aiEvaluationService.setModel(currentModel);
      }

      setTestResult({ success: true, message: 'Connexion réussie ! L\'API fonctionne correctement.' });
    } catch (error) {
      const currentProvider = localStorage.getItem('ai_provider');
      const currentApiKey = localStorage.getItem('ai_api_key');
      if (currentProvider) {
        aiEvaluationService.setProvider(currentProvider as AIProvider, currentApiKey || '');
      }

      const msg = error instanceof Error ? error.message : 'Erreur inconnue';
      setTestResult({ success: false, message: `Erreur de connexion: ${msg}` });
    } finally {
      setIsTestingConnection(false);
    }
  };

  React.useEffect(() => {
    if (isOpen) {
      const savedProvider = localStorage.getItem('ai_provider') as AIProvider;
      const savedApiKey = localStorage.getItem('ai_api_key');
      const savedModel = localStorage.getItem('ai_model');

      if (savedProvider) {
        setSelectedProvider(savedProvider);
        setSelectedModel(savedModel || DEFAULT_MODELS[savedProvider] || '');
      }
      if (savedApiKey) {
        setApiKey(savedApiKey);
      }
    }
  }, [isOpen]);

  if (!isOpen) return null;

  const selectedProviderInfo = providers.find(p => p.id === selectedProvider);
  const models = PROVIDER_MODELS[selectedProvider] || [];

  const apiKeyPlaceholders: Record<string, string> = {
    chatgpt: 'sk-...',
    anthropic: 'sk-ant-...',
    gemini: 'AIza...',
    mistral: 'votre clé Mistral',
  };

  const apiKeyLinks: Record<string, { label: string; href: string }> = {
    chatgpt: { label: 'OpenAI Platform', href: 'https://platform.openai.com/api-keys' },
    anthropic: { label: 'Anthropic Console', href: 'https://console.anthropic.com/settings/keys' },
    gemini: { label: 'Google AI Studio', href: 'https://makersuite.google.com/app/apikey' },
    mistral: { label: 'console.mistral.ai', href: 'https://console.mistral.ai/' },
  };

  return (
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
      <div className="relative top-4 mx-auto p-5 border w-full max-w-2xl shadow-lg rounded-md bg-white mb-8">
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-xl font-bold text-gray-900 flex items-center">
            <Brain className="h-6 w-6 mr-2 text-primary-600" />
            Configuration de l'IA d'évaluation
          </h3>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600 transition-colors">
            <X className="h-6 w-6" />
          </button>
        </div>

        <Card>
          <CardHeader>
            <CardTitle>Choisir le fournisseur d'IA</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            {providers.map(provider => (
              <label
                key={provider.id}
                className={`flex items-center p-4 border rounded-lg cursor-pointer transition-colors ${
                  selectedProvider === provider.id
                    ? 'border-primary-500 bg-primary-50'
                    : 'border-gray-200 hover:bg-gray-50'
                }`}
              >
                <input
                  type="radio"
                  name="provider"
                  value={provider.id}
                  checked={selectedProvider === provider.id}
                  onChange={() => handleProviderChange(provider.id)}
                  className="sr-only"
                />
                <div className={`mr-3 ${provider.color}`}>{provider.icon}</div>
                <div className="flex-1">
                  <div className="font-medium text-gray-900">{provider.name}</div>
                  <div className="text-sm text-gray-500">{provider.description}</div>
                  {provider.requiresApiKey && (
                    <div className="text-xs text-warning-600 mt-1">Nécessite une clé API</div>
                  )}
                </div>
                {selectedProvider === provider.id && (
                  <div className="w-4 h-4 bg-primary-600 rounded-full flex items-center justify-center">
                    <div className="w-2 h-2 bg-white rounded-full" />
                  </div>
                )}
              </label>
            ))}

            {selectedProviderInfo?.requiresApiKey && (
              <div className="mt-6 p-4 bg-gray-50 rounded-lg space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    <Key className="h-4 w-4 inline mr-1" />
                    Clé API {selectedProviderInfo.name}
                  </label>
                  <input
                    type="password"
                    value={apiKey}
                    onChange={(e) => setApiKey(e.target.value)}
                    className="block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm"
                    placeholder={apiKeyPlaceholders[selectedProvider] || '...'}
                  />
                  {apiKeyLinks[selectedProvider] && (
                    <p className="mt-1 text-xs text-gray-500">
                      Obtenez votre clé API sur{' '}
                      <a
                        href={apiKeyLinks[selectedProvider].href}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-primary-600 hover:underline"
                      >
                        {apiKeyLinks[selectedProvider].label}
                      </a>
                    </p>
                  )}
                </div>

                {models.length > 0 && (
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      Modèle
                    </label>
                    <select
                      value={selectedModel}
                      onChange={(e) => setSelectedModel(e.target.value)}
                      className="block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm"
                    >
                      {models.map(m => (
                        <option key={m.value} value={m.value}>{m.label}</option>
                      ))}
                    </select>
                  </div>
                )}

                {apiKey && (
                  <Button
                    type="button"
                    variant="outline"
                    size="sm"
                    onClick={handleTestConnection}
                    isLoading={isTestingConnection}
                    leftIcon={<TestTube className="h-4 w-4" />}
                  >
                    Tester la connexion
                  </Button>
                )}

                {testResult && (
                  <div className={`p-3 rounded-md ${
                    testResult.success
                      ? 'bg-success-50 text-success-800 border border-success-200'
                      : 'bg-error-50 text-error-800 border border-error-200'
                  }`}>
                    <div className="text-sm">{testResult.message}</div>
                  </div>
                )}
              </div>
            )}
          </CardContent>
          <CardFooter className="flex justify-end space-x-3">
            <Button variant="outline" onClick={onClose}>
              Annuler
            </Button>
            <Button
              variant="primary"
              onClick={handleSave}
              leftIcon={<Save className="h-4 w-4" />}
              disabled={!!(selectedProviderInfo?.requiresApiKey && !apiKey)}
            >
              Enregistrer
            </Button>
          </CardFooter>
        </Card>
      </div>
    </div>
  );
});

AIConfigModal.displayName = 'AIConfigModal';

export default AIConfigModal;
